...
def snazzyFeatureExtractor(state, action):
   ...
   #features.append(("hascards_"+str(hasCardList), 1))
    ...
    nextind = nplayers if len(state[1]) > nplayers else nplayers % len(state[1])
    features.append(('hasnextcard', 1 if state[1][nextind] > 0 else 0)) # do we have the next card to play
    features.append(("nnextcard_"+str(state[1][nextind]), 1))

    # if len(state[1]) > nplayers:
    #     features.append(('hasnextcard', 1 if state[1][nplayers] > 0 else 0)) # do we have the next card to play
